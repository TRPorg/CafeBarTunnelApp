...
final class ReportAnualItemCell: UITableViewCell {
    ...
    func configure(model: CheckoutItemModel) {
        ...
        if model.volume != 0 {
            itemPriceLabel.text = "Всего:\(model.quantity) Объем:\(model.volume * model.quantity)мл Сумма:\(model.price * model.quantity)руб."
            return
        }
        itemPriceLabel.text = "Всего:\(model.quantity) Сумма:\(model.price * model.quantity)руб."
    }
    ...
    private func setUpLayout() {
        ...
        NSLayoutConstraint.activate([
            itemNameLabel.topAnchor.constraint(equalTo: contentView.topAnchor, constant: 30),
            ...
            itemNameLabel.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -10),

            itemPriceLabel.leftAnchor.constraint(equalTo: itemNameLabel.rightAnchor),
